...
import React, { useState } from "react";
import { generateRSAKeyPairHex } from "../utils/cryptoHelpers";
import "../styles/keytools.css"; // custom styles
...
const KeyTools = () => {
  ...
  return (
    <div className="keytools-container">
      <h2 className="keytools-title">🔑 Key Tools</h2>

      <div className="keytools-actions">
        <button onClick={handleGenerateKeys} className="btn-primary">
          Generate Key Pair
        </button>
        <button onClick={handleClearKeys} className="btn-danger">
          Clear Keys
        </button>
      </div>
      ...
      {publicKey && (
        <div className="key-block">
          <h3>Public Key</h3>
          <textarea readOnly value={publicKey} className="key-textarea" />
          <button
            onClick={() => handleCopy(publicKey)}
      ...
      {privateKey && (
        <div className="key-block">
          <h3>Private Key</h3>
          <textarea readOnly value={privateKey} className="key-textarea" />
          <button
            onClick={() => handleCopy(privateKey)}
            className="btn-secondary"
          >
